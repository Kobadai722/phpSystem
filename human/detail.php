...
<body>
    <h1>
        <?php
        // Warning: Undefined array key "NAME" 対策
        if (isset($_GET['NAME']) && !empty($_GET['NAME'])) {
            echo htmlspecialchars($_GET['NAME']) . "-詳細";
        } else {
            echo "社員詳細"; // NAMEパラメータがない場合のデフォルトタイトル
        }
        ?>
    </h1>
    ...
    <table class="table table-hover">
        ...
        ?>
                ...
                <?php
                    ...
                    echo '<tr><td colspan="8">該当する社員情報が見つかりませんでした。</td></tr>';
                }
            } catch (PDOException $e) {
                // データベース関連のエラーが発生した場合の処理
                // 開発中はエラーメッセージを表示するとデバッグに役立ちます
                // echo '<tr><td colspan="8">データベースエラー: ' . htmlspecialchars($e->getMessage()) . '</td></tr>';
                // 本番環境では、より一般的なメッセージを表示するか、ログに記録します
                error_log("Database Error: " . $e->getMessage()); // エラーログに記録
                echo '<tr><td colspan="8">情報の取得中にエラーが発生しました。管理者に連絡してください。</td></tr>';
            }
        } else {
            // NAMEパラメータがない場合の処理
